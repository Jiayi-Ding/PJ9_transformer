...
"""【v11】
 ...
 --use_pingze: 启用平仄约束
 修复：律诗后四句约束失效；自动根据起笔字选择平起/仄起格式。
"""

import argparse
import os
import sys
import json   # 修复 json 未定义错误
...
def generate_one(
    ...
    if use_pingze and pingze_helper is not None:
        ...
        if genre == "5":
            if first_pingze == 0:   # 平声 -> 平起
                pingze_template = PINGZE_TEMPLATE_5_PINGQI
                print(f"自动选择五言平起格式（起笔字「{first_char}」为平声）")
            else:                    # 仄声 -> 仄起
                pingze_template = PINGZE_TEMPLATE_5
                print(f"自动选择五言仄起格式（起笔字「{first_char}」为仄声）")
        else:   # genre == "7"
            if first_pingze == 0:   # 平声 -> 平起
                pingze_template = PINGZE_TEMPLATE_7
                print(f"自动选择七言平起格式（起笔字「{first_char}」为平声）")
            else:                    # 仄声 -> 仄起
                pingze_template = PINGZE_TEMPLATE_7_ZQI
                print(f"自动选择七言仄起格式（起笔字「{first_char}」为仄声）")
    # ==============================================
    ...
    for _ in range(max_new):
        ...
        if use_pingze and pingze_helper is not None and pingze_template is not None:
            # 当前正在生成的是中文字符（不是标点）
            # 计算当前句内位置（下一字的索引，0-based）
            pos = chinese_in_line
            # 行未满且尚未超过模板长度（使用循环索引以支持律诗）
            if pos < line_length:
                template_idx = line_count % len(pingze_template)
                expected_pingze = pingze_template[template_idx][pos]

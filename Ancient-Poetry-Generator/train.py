# -*- coding: utf-8 -*-
"""训练脚本模块，包含模型训练流程与验证工具。

本模块支持基于 PoetryBlockDataset 的训练、可选主题 embedding、
torch.compile 编译、混合精度训练、学习率调度与损失曲线保存。
"""
...
def main() -> None:
    """训练入口，解析参数并执行训练、验证与模型保存。

    支持配置训练集/验证集路径、模型结构参数、主题 embedding、
    torch.compile、混合精度、梯度裁剪和学习率调度。
    """
    ...
    if args.use_topic:
        ...
        boundaries_train_path = f"boundaries_train_{args.genre}.pt"
        boundaries_val_path = f"boundaries_val_{args.genre}.pt"

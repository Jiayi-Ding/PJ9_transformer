...
def generate_one(
    ...
    # 如果是单字起笔且词表包含换行符，则在起笔前插入换行符，引导模型从新行开始作诗
    newline_id = stoi.get("\n", None)
...
def main() -> None:
    ...
    p.add_argument(
        "--list-cipai",
        action="store_true",
        help="列出 cipai_plan.json 中可训练的词牌/合并组",
    )
    p.add_argument(
        "--poetry_name",
        type=str,
        default=None,
        help="词牌文件前缀（如 huanxisha、merge_short）；指定后加载对应模型",
    )
    p.add_argument(
        "--cipai",
        action="store_true",
    ...
    if args.list_cipai:
        ...
        if not items:
            print("未找到 cipai_plan.json，请先运行 analyze_cipai.py", file=sys.stderr)
            sys.exit(1)

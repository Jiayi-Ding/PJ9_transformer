...
def generate_one(
    ...
    for _ in range(max_new):
        nxt = sample_next(model, torch.tensor([out_ids], device=device, dtype=torch.long), topic_id, temperature, device)
        out_ids.append(nxt)
...
def auto_select_topic(prompt: str, topic_vocab_path: str) -> str:
    ...
    topic_list = topic_data.get("topics", ["frontier", "landscape", "farewell", "other"])
...
def main():
    ...
    p.add_argument("--topic", type=str, choices=["frontier", "landscape", "farewell", "other"], 
                   default="other", help="主题: frontier/landscape/farewell/other")
